...
if isempty(sliceTimes)
    sliceTimes = checkMRIProperty('ST',inputPaths{1});
    if isempty(sliceTimes)
...
if isempty(tr)
    tr = checkMRIProperty('TR',inputPaths{1});
    if isempty(tr)
...
[inputDir,~,~] = filepartsGZ(inputPaths{1});
bidsBaseDir = bidsCheckBaseDir(inputDir);  % BIDS root directory
if isempty(bidsBaseDir), bidsBaseDir = '123059815310419841xyz'; end     % Hack so that strrep command below works for non-BIDS input
...
for e=1:length(inputPaths)
    outputPaths{e} = bidsChangeEntity(inputPaths{e},'desc',outputDescShort);
    system(['slicetimer -i ' inputPaths{e} ' -o ' outputPaths{e} ' --tcustom=' stFilePath]);
    jsonReconstruct(strrep(inputPaths{e},'.nii.gz','.json'),strrep(outputPaths{e},'.nii.gz','.json'));
    jsonChangeValue(strrep(outputPaths{e},'.nii.gz','.json'),{'Description','Sources'},...

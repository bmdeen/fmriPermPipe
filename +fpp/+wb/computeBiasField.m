...
% rough brain mask.
%
% fpp.wb.computeBiasField(inputT1Path,inputT2Path,maskPath,outputBiasPath...
%   [,outputDescription,appendDescription])
%
% Arguments:
% - inputT1Path (string): path to input T1 image
% - inputT2Path (string): path to input T2 image
% - maskPath (string): path to input brain mask image
% - outputBiasPath (string): path to output bias field image
%
% Optional arguments:
...
function computeBiasField(inputT1Path,inputT2Path,maskPath,outputBiasPath,outputDescription,appendDescription)
...
fpp.util.system(['cp ' maskPath ' ' tmpDir '/mask' extMask]);
maskPath = [tmpDir '/mask' extMask];

% Form sqrt(T1w*T2w), mask, and normalise by the mean
fpp.fsl.maths(inputT1Path,['-mul ' inputT2Path ' -abs -sqrt'],[tmpDir '/T1wmulT2w.nii.gz']);
fpp.fsl.maths([tmpDir '/T1wmulT2w.nii.gz'],['-mas ' maskPath],[tmpDir '/desc-brain_T1wmulT2w.nii.gz']);
[~,meanBrainVal] = fpp.util.system(['fslstats ' tmpDir '/desc-brain_T1wmulT2w.nii.gz -M']);
meanBrainVal = str2num(strtrim(meanBrainVal));
fpp.fsl.maths([tmpDir '/desc-brain_T1wmulT2w.nii.gz'],['-div ' ...
...
fpp.fsl.maths([tmpDir '/desc-brainNorm_T1wmulT2w.nii.gz'],['-bin -s ' smoothingSigma],...
    [tmpDir '/desc-Sm' smoothingSigma '_mask.nii.gz']);
fpp.fsl.maths([tmpDir '/desc-brainNorm_T1wmulT2w.nii.gz'],['-s ' smoothingSigma ...
...
fpp.fsl.maths([tmpDir '/desc-brainNorm_T1wmulT2w.nii.gz'],...
...
fpp.fsl.maths([tmpDir '/desc-brainNormModulate_T1wmulT2w.nii.gz'],...
...
fpp.fsl.maths([tmpDir '/desc-brainNorm_T1wmulT2w.nii.gz'],...
    ...
    [tmpDir '/bias.nii.gz']);
fpp.fsl.maths([tmpDir '/bias.nii.gz'],['-s ' smoothingSigma],[tmpDir '/bias.nii.gz']);

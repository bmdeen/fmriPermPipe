...
% - nComps (integer in [0,Inf)): number of aCompCorr PCs, from WM and CSF
%       each, to include in confound file
% - teVals (vector of values in (0,Inf)): TE values (ms) for multi-echo 
%       data (default: read from json)
% - echoForMoCorr (integer in (0,Inf), default=2): echo number to use for
%       motion correction
% - tedPCA (string): TEDANA tedpca input, determines method for PCA comp
%       selection. Options: mdl, kic, aic (more to less aggressive)
% - manAcc (vector of integers in [0,Inf)): TEDANA components to manually
%       accept, if re-running
...
% - useDespike (boolean; default=1): whether to use AFNI's 3dDespike
...
tedPCA = 'mdl';                 % TEDANA tedpca input, method to select components. Options: mdl, kic, aic
manAcc = [];                    % TEDANA components to manually accept, if rerunning.
...
varArgList = {'overwrite','funcTemplatePath','funcTemplateSpace','fwhm',...
    ...
    'rotCutoff','tptsAfter','disdaqs','nComps','genTSNR','outputOptcomb',...
    'useSTC','useTedana','echoForMoCorr','undistort','fdCutoff'...
...
%     if ~exist(outputPaths{1},'file') %%% TEMPORARY DEBUGGING HACK
    fpp.func.preproc.tedana(inputPaths,outputPaths{1},maskNonZeroPath,[],1,...
        teVals,'tedPCA',tedPCA,'manAcc',manAcc);
%     end
elseif multiEcho

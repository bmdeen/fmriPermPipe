...
if ~exist('spinEchoPhaseEncodeDirections','var') || isempty(spinEchoPhaseEncodeDirections)
    for f=1:length(spinEchoPaths)
        tmp = fpp.util.checkMRIProperty('PEDir',spinEchoPaths{f});
        if ~isempty(tmp)
...
for f=1:length(spinEchoPaths)
    tmp = fpp.util.checkMRIProperty('PEDirStr',spinEchoPaths{f});
    if ~isempty(tmp)
...
if ~exist('funcDataPhaseEncodeDirection','var') || isempty(funcDataPhaseEncodeDirection)
    tmp = fpp.util.checkMRIProperty('PEDir',inputFuncPath);
    if ~isempty(tmp)

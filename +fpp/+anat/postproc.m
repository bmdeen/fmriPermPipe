...
% Check for ASHS segmentation
ashsPath = fpp.bids.changeName(wmPath,'desc','ashs');
...
for h=1:2
    ...
    if ~exist([msmDir '/sub-' subjID '_hemi-' Hemis{h} '_desc-msm_sphere.reg.surf.gii'],'file') %%% TEMPORARY FOR DEBUGGING
        fpp.util.system(['msm --conf=' dataDir '/MSMSulcStrainFinalconf'...
...
% Define ASHS masks in funcRes space
if ashsExists
    ashsFuncResPath = fpp.bids.changeName(ashsPath,'res',funcResolution);
    hemis = {'L','R',''};
    for i=1:length(segInd)
        for h=1:length(hemis)
            ...
            outputMaskPath = fpp.bids.changeName(ashsFuncResPath,'desc',['ashs' hemis{h} segNames{i}],'mask');
            fpp.util.label2ROI(ashsFuncResPath,segInd{i},outputMaskPath);
            fpp.bids.jsonChangeValue(outputMaskPath,{'Type','Description','Sources'},...
                ...
                ' of Hippocampal Subfields'],ashsFuncResPath});
        end
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% STEP 12.5 Resample ASHS seg to surface
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ashsExists
    ...
    for h=1:2
        ...
        inputSurfaceFsLRPaths{h} = {midthickFsLRPaths{h},fpp.bids.changeName(midthickPaths{h},[],[],'white'),...
            fpp.bids.changeName(midthickPaths{h},[],[],'pial')};
    end
    subcortSegIndivFuncResPath = fpp.bids.changeName(subcortSegIndivPath,'res',funcResolution);
    fpp.func.surfaceResample(ashsPath,inputSurfacePaths{h},maskAtlasPaths,ashsCiftiPath,...
        'subcortSegPath',subcortSegIndivFuncResPath,'isLabel',1,...
        'referenceFuncResPath',subcortSegIndivFuncResPath,'mapNames','ASHS');
    fpp.func.surfaceResample(ashsPath,inputSurfaceFsLRPaths{h},maskAtlasFsLRPaths,ashsCiftiFsLRPath,...
        'subcortSegPath',subcortSegIndivFuncResPath,'isLabel',1,...

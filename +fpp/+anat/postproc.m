...
% - Add BIDS metadata for label files
...
for p=1:length(parcs)
    ...
    % HERE: Add json files for parcellation volumes
end
% Define brain mask
maskPath = fpp.bids.changeName(inputT1Path,{'desc','res'},{'brainFS',[]},'mask','.nii.gz');
fpp.fsl.fslMaths(wmPath,'-bin -dilD -dilD -dilD -ero -ero',maskPath);
fpp.wb.command('volume-fill-holes',maskPath,[],maskPath);
fpp.fsl.fslMaths(maskPath,'-bin','maskPath');
...
fpp.fsl.moveImage(vmPFCMaskPathStd,inputT1Path,vmPFCMaskPath,[],'warp',...
    standard2IndividualXfmInit,'rel',1,'interp','nn');  % Generate json file for vmPFC mask?
fpp.fsl.fslMaths(maskPath,['-add ' vmPFCMaskPath ' -bin'],maskPath);
% Mask brain images
fpp.fsl.fslMaths(inputT1Path,['-mul ' maskPath],fpp.bids.changeName(inputT1Path,'desc','preprocBrain'));
if exist(inputT2Path,'file')
    fpp.fsl.fslMaths(inputT2Path,['-mul ' maskPath],fpp.bids.changeName(inputT2Path,'desc','preprocBrain'));
end
% Generate dilated brain mask to convert to func template space
fpp.fsl.fslMaths(maskPath,'-dilD',fpp.bids.changeName(maskPath,'desc','brainFSdil1'));
...
fpp.util.system(['cp ' dataDir '/*_space-fs* ' anatPreprocDir '/']);    % Copy fsLR atlas files
...
if exist(hcpAtlasDir,'dir')
    ...
    for p=1:length(parcNames)   % Parcellations
        ...
        if sum(regexp(parcNames{p},'medialwall'))==0    % Convert parc to volume
           ...
           fpp.fsl.fslMaths(parcVolPaths{1},['-add ' parcVolPaths{2}],parcVolPath);
...
if ~exist(hcpAtlasDir,'file'), surfaceSpaces = surfaceSpaces(1:2); end  % Only use S1200 space if atlas files exist
...
parcNames = {'medialwall','medialwallConte69','aparc','aparc.a2009s','MMP','RSN','Gordon'};
for s=1:length(surfaceSpaces)
    ...
    for i=1:length(metrics)
        ...
        fpp.wb.command('add-to-spec-file',specPath,structures{h},fpp.bids.changeName(sulcPaths{h},...
    ...
    for i=1:length(parcNames)
        subLabel = subjID;
        if ismember(parcNames{i},{'medialwallConte69','MMP','RSN','Gordon'}) ...
    ...
    % HERE: add border files, annot for third SPEC
end



% TODO HERE:
% - Generate cortical ribbon ROI
% - Generate myelin map?

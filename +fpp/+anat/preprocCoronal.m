...
if exist(ashsDir,'dir')
    hemis = {'L','R',''};
    outputSegDir = fpp.bids.changeName(outputCoronalPath,[],[],'ashs','');
    outputSegPath = fpp.bids.changeName(outputCoronalPath,'desc','ashs','dseg');    % bilateral
    % Perform ASHS segmentation
    fpp.util.system(['ashs_main.sh -I sub-' subjID ' -a ' ashsDir ' -g ' preprocT1InCoronalPath...
        ' -f ' outputCoronalPath ' -w ' outputSegDir]);
    % Copy and rename output segmentation
    fpp.fsl.maths([outputSegDir '/final/sub-' subjID '_right_lfseg_corr_nogray.nii.gz'],...
        ['-add 100 -add ' outputSegDir '/final/sub-' subjID '_left_lfseg_corr_nogray.nii.gz'],...
        outputSegPath);
    ...
    for i=1:length(segInd)
        for h=1:length(hemis)
            ...
            fpp.util.label2ROI(outputSegPath,segInd{i},outputMaskPath);
...
for i=1:length(segInd)
    for h=1:length(hemis)
        ...
        fpp.util.label2ROI(outputSegIndivPath,segInd{i},outputMaskPath);
        fpp.bids.jsonChangeValue(outputMaskPath,{'Type','Description','Sources'},...
            {'ROI',[hemis{h} '_' segNames{i} ' mask generated by Automatic Segmentation'...

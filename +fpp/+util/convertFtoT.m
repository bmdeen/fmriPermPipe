...
for e=1:length(extensions)
    if contains(fMapPath,extensions{e})
        system(['mri_convert ' fMapPath ' ' strrep(fMapPath,extensions{e},'.nii.gz')]);
        fMapPath = strrep(fMapPath,extensions{e},'.nii.gz');
    end
    if contains(sigMapPath,extensions{e})
        system(['mri_convert ' sigMapPath ' ' strrep(sigMapPath,extensions{e},'.nii.gz')]);
        sigMapPath = strrep(sigMapPath,extensions{e},'.nii.gz');
    end
end

system(['fslmaths ' sigMapPath ' -div 1000 -bin ' fileRoot '-posVals.nii.gz']);
system(['fslmaths ' sigMapPath ' -div 1000 -mul -1 -bin ' fileRoot '-negVals.nii.gz']);
system(['fslmaths ' fMapPath ' -sqrt -mul ' fileRoot '-posVals.nii.gz ' fileRoot '-posTVals.nii.gz']);
system(['fslmaths ' fMapPath ' -sqrt -mul ' fileRoot '-negVals.nii.gz ' fileRoot '-negTVals.nii.gz']);
system(['fslmaths ' fileRoot '-posTVals.nii.gz -sub ' fileRoot '-negTVals.nii.gz ' outputPath]);
system(['rm -rf ' fileRoot '-posVals.nii.gz ' fileRoot '-negVals.nii.gz ' ...

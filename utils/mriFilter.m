...
inputData = MRIread2(inputPath);
outputData = inputData;

dims = size(inputData.vol);
inputMat = reshape(inputData.vol,[prod(dims(1:3)) dims(4)])';
outputMat = firFilter(inputMat,filtCutoff,1/tr,type,filtOrder) + repmat(mean(inputMat),[size(inputMat,1) 1]);

outputData.vol = reshape(outputMat',dims);
MRIwrite2(outputData,outputPath);

...
% FSL source prefix, not currently implemented
fslPrefix = '';

if isempty(strfind(inputPath,'.nii.gz'))
...
[outputDir,inputName,inputExt] = fileparts(inputPath);
...
[~,vols] = system([fslPrefix 'fslval ' inputPath ' dim4']);
vols = str2num(strtrim(vols));
volsPerTE = vols/numTEs;
tr = determineTR(inputPath,0,2,fslPrefix);
system([fslPrefix 'fslsplit ' inputPath ' ' tmpDir '/split -t']);
for e=1:numTEs
    sepTEPaths{e} = [tmpDir '/rawdata' teSuffices{e} '.nii.gz'];
    mergeCmd{e} = [fslPrefix 'fslmerge -tr ' sepTEPaths{e}];
end
for t=1:vols
...
for e=1:numTEs
    ...
    system([fslPrefix 'fslmaths ' sepTEPaths{e} ' -Tmean ' meanFuncSep{e}]);
end
...
for e=1:numTEs
    funcSepData{e} = MRIread(sepTEPaths{e});
end
...
MRIwrite(newData,outputPath,'short');

...
%   defaultSuffix (string, optional) - suffix to assume if input is not
%       BIDs-formatted
%   newExtension (string, optional) - new file extension

function outputPath = bidsChangeEntity(inputPath,keysToChange,newValues,defaultSuffix,newExtension)
...
if ~exist('defaultSuffix','var') || ~ischar(defaultSuffix)
    defaultSuffix = 'bold';
end
if ~isempty(inputPath)
    [inputDir,inputName,inputExtension] = filepartsGZ(inputPath);
else
    inputDir = '';
    inputName = '';

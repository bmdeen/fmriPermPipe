...
% preprocMRI(inputPaths,outputDir,varargin)
% 
...
% 
% Example usage:
% preprocMRI('/studyDirectory/SUB1/func/task-1.nii.gz',...
%   '/studyDirectory/SUB1/analysis/task-1.prep','fwhm',4,'plotResults',0);
% 
...
% - Add JSON files to mocotarget, undistorted mocotarget, func template
%
% TODO EVENTUALLY:
...
function preprocMRI(inputPaths,outputDir,varargin)
...
varArgList = {'overwrite','funcTemplatePath','funcTemplateName','fwhm',...
    ...
    'stdCutoff','disdaqs','genTSNR','plotResults',...
...
if multiEcho
    if isempty(teVals)
        teVals = checkMRIProperty('TE',inputPaths{1});
        if isempty(teVals)
...
[inputDir,inputName,~] = filepartsGZ(inputPaths{1});
if isempty(inputDir), inputDir = pwd; end
finalOutputPath = bidsChangeEntity([funcPreprocDir '/' inputName '.nii.gz'],{'desc','echo'},{'preproc',[]});
...
bidsBaseDir = bidsCheckBaseDir(outputDir);  % BIDS root directory
...
for e=1:nEchoes
    [~,inputNames{e},~] = filepartsGZ(inputPaths{e});
    outputPaths{e} = [funcPreprocDir '/' inputNames{e} '.nii.gz'];
    copyImageAndJson(inputPaths{e},outputPaths{e});
end

% Copy and validate spin echo "field map" images, if undistorting
if undistort
    [errorMsg,spinEchoPaths,fieldMapParamPath] = copyAndCheckSpinEcho(inputPaths{1},...
...
if isempty(funcTemplatePath)
    if useTaskTemplate
        funcTemplatePath = [funcPreprocDir '/' bidsChangeEntity(inputName,{'desc','run','echo'},{'FuncTemplate','',''}) '.nii.gz'];
        funcTemplateName = 'task';
    else
        funcTemplatePath = [funcPreprocDir '/' bidsChangeEntity(inputName,{'desc','task','acq','run','echo'},{'FuncTemplate','','','',''}) '.nii.gz'];
...
% Check TR
tr = checkMRIProperty('tr',outputPaths{1});
if isempty(tr)
...
motionParams = estimateHeadMotion(inputPaths{echoForMoCorr},mcDir,moCorrTargetVolNum);
artifactTPs = defineMotionArtifactTimePoints(motionParams,transCutoff,rotCutoff,tptsAfter);
...
for e=1:nEchoes
    outputPaths{e} = bidsChangeEntity(outputPaths{e},'desc','midprep1despike');
...
if useSTC
    ...
    inputPaths = outputPaths;
    [errorMsg,outputPaths] = sliceTimingCorrect(inputPaths,sliceTimes,tr);
    if ~isempty(errorMsg)
        fprintf('%s\n\n',errorMsg);
        return;
    end
...
mocoTargetPath = bidsChangeEntity(outputPaths{echoForMoCorr},'desc','MocoTargetVol');
system(['fslroi ' outputPaths{echoForMoCorr} ' ' mocoTargetPath ' ' int2str(moCorrTargetVolNum) ' 1']);
% Undistorted MocoTarget vol
if undistort
    mocoTargetUndistortedPath = bidsChangeEntity(outputPaths{echoForMoCorr},'desc','MocoTargetVolUndistorted');
    [errorMsg,topupWarpPath,topupJacobianPath,xfmMocoTarget2SpinEcho,xfmSpinEcho2MocoTarget] = ...
        undistortMRI(mocoTargetPath,mocoTargetUndistortedPath,spinEchoPaths,fmapPreprocDir,fieldMapParamPath);
    if ~isempty(errorMsg)
        fprintf('%s\n\n',errorMsg);
        return;
    end
...
jsonChangeValue(convertNiiJson(funcTemplatePath),{'Description','TaskName','TaskDescription',...
...
initMaskPath = strrep(bidsChangeEntity(funcTemplatePath,'desc','FuncTemplateInitMask'),'_bold.nii.gz','_mask.nii.gz');
system(['bet2 ' funcTemplatePath ' ' strrep(initMaskPath,'.nii.gz','') ' -f ' num2str(faValue) ' -m -n']);
jsonReconstruct(convertNiiJson(funcTemplatePath),convertNiiJson(initMaskPath),{'SpatialReference'});
jsonChangeValue(convertNiiJson(initMaskPath),{'Description','Sources','Type'},...
...
xfmMocoTarget2FuncTemplate = strrep(bidsChangeEntity(mocoTargetPath,{'desc','from','to','mode'},...
    {'','orig','session','image'}),'_bold.nii.gz','_xfm.mat');
xfmFuncTemplate2MocoTarget = strrep(bidsChangeEntity(mocoTargetPath,{'desc','from','to','mode'},...
    {'','orig','session','image'}),'_bold.nii.gz','_xfm.mat');
...
inputPaths = outputPaths;
for e=1:nEchoes
    outputPaths{e} = bidsChangeEntity(outputPaths{e},'desc','midprep3moco');
end
oneShotMotionDistortionCorrect(inputPaths,outputPaths,mcDir,funcTemplatePath,funcTemplateName,topupWarpPath,...
    topupJacobianPath,xfmMocoTarget2FuncTemplate,xfmSpinEcho2MocoTarget,xfmMocoTarget2SpinEcho);
...
if multiEcho
    inputPaths = outputPaths;
    outputPath = bidsChangeEntity(inputPaths{1},{'echo','desc'},{[],'midprep5optcomb'});
    ...
    jsonReconstruct(convertNiiJson(inputPaths{1}),convertNiiJson(outputPath));
    jsonChangeValue(convertNiiJson(outputPath),{'Description','Sources','EchoTime','EchoNumber'},...
...
outputPath = bidsChangeEntity(inputPath,'desc',{[],'midprep6intnorm'});
...
jsonReconstruct(convertNiiJson(inputPath),convertNiiJson(outputPath));
jsonChangeValue(convertNiiJson(outputPath),{'Description','Sources'},...
...
if tempFilt
    inputPath = outputPath;
    outputPath = bidsChangeEntity(inputPath,'desc',{[],'midprep7tempfilt'});
...
if fwhm>0
    inputPath = outputPath;
    outputPath = bidsChangeEntity(inputPath,'desc',{[],'midprep8smooth'});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
inputPath = outputPath;
outputPath = bidsChangeEntity(inputPath,'desc',{[],'preproc'});
system(['mv ' inputPath ' ' outputPath]);
system(['mv ' convertNiiJson(inputPath) ' ' convertNiiJson(outputPath)]);
jsonChangeValue(convertNiiJson(outputPath),{'Description'},{'Preprocessed data generated by fmriPermPipe.'});
